...
def rotation_calculation_with3x3matrix( vectorBefore , vectorAfter  ) :
    ...
    rotationangle=m.acos(np.dot(vectorBefore,vectorAfter)/(np.linalg.norm(vectorBefore)*np.linalg.norm(vectorAfter)))
    matrix=angle2rotm(rotationangle, rotation_axis)